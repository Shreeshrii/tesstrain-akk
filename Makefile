...
# Doesn't seem to work - include in bash script
###ifdef $(START_MODEL)
...
$(OUTPUT_DIR)/unicharset: $(ALL_GT)
	...
	merge_unicharsets data/$(START_MODEL)/$(START_MODEL).lstm-unicharset $(OUTPUT_DIR)/my.unicharset  "$@"
else
$(OUTPUT_DIR)/unicharset: $(ALL_GT)
	@mkdir -p $(OUTPUT_DIR)
...
# Generate box. lstmf, all-lstmf and all-gt via bash script
#
